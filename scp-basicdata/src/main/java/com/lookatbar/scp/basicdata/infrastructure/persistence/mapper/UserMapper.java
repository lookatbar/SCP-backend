...
public interface UserMapper extends BaseMapper<UserPO> {
    ...
    @Select("SELECT * FROM rbac_user WHERE email = #{email}")
    Optional<UserPO> findByEmail(@Param("email") String email);

    @Select("SELECT * FROM rbac_user WHERE phone = #{phone}")
    Optional<UserPO> findByPhone(@Param("phone") String phone);

    @Select("SELECT * FROM rbac_user WHERE status = #{status}")
    List<UserPO> findByStatus(@Param("status") Integer status);

    @Select("SELECT COUNT(*) FROM rbac_user WHERE username = #{username}")
    int countByUsername(@Param("username") String username);

    @Select("SELECT COUNT(*) FROM rbac_user WHERE email = #{email}")
    int countByEmail(@Param("email") String email);

    @Select("SELECT COUNT(*) FROM rbac_user WHERE phone = #{phone}")
